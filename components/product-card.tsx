...
export default function ProductCard({
  ...
  return (
    <div className="group relative">
      {badge && (
        <span className="absolute left-2 top-2 z-10 bg-[#f34f38] px-3 py-1 text-xs text-white rounded-full shadow-sm">
          {badge}
        </span>
      )}
      <Link href={href} className="block">
        <div className="relative mb-3 overflow-hidden rounded-2xl bg-white shadow-sm">
          ...
          <button className="absolute right-2 top-2 rounded-full bg-white p-2 opacity-0 transition-opacity duration-300 group-hover:opacity-100 shadow-sm">
            <Heart className="h-4 w-4 text-[#f34f38]" />
          </button>
          <div className="absolute inset-0 bg-gradient-to-t from-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
        </div>
      </Link>
      <h3 className="mb-1 text-sm font-medium">{title}</h3>
      <p className="mb-2 font-semibold text-[#f34f38]">{price}</p>
      {colors && (
